...
커널 -> 중개인
접근 불가능이어야 할 프로그램이 장치에 접근하는 문제를 해결하기 위해 필요

하드웨어를 통해 프로세스가 장치에 직접 접근할 수 없게 함
	CPU에 내장된 모드 사용(커널 모드와 사용자 모드)
커널 모드로 동작하면서 다른 프로세스에서는 불가능한 장치 제어

### 시스템 콜
프로세스가 커널에 처리를 요청하는 방법
프로세스가 시스템 콜을호출하면 CPU 에서는 예외 이벤트 발생

### 라이브러리
표준 C 라이브러리
	C언어의 경우 ISO에서 정한 표준 라이브러리 존재
시스템 콜 래퍼 함수
glibc

정적 라이브러리와 공유(동적) 라이브러리
...
메모리 어드레스가 랜덤하게 실행되기 때문에 고정적 주소가 아닌 상대적 주소로만 접근하여 실행할 수 있게 하게끔 하여 공격자가 접근하기 어렵게 함 -> Mitigation

Idle 상태?

### 좀비 프로세스와 고아 프로세스
좀비 프로세스
종료했지만 부모가 종료 상태를 확인하지 않은 상태의 프로세스

고아 프로세스
부모 프로세스가 먼저 종료됐을 때 이에 해당하는 자식 프로세스
(커널은 init을 고아 프로세스의 새로운 부모로 지정)
-> init 프로세스는 0차 부모 프로세스

### 셀 작업 관리 구현
세션
사용자가 단일 애뮬레이터 또는 ssh 등을 사용해서 시스템에 로그인했을 때의 로그인 세션

### 데몬
상주하는 프로세스, 즉 시스템 시작부터 종료때까지 실행

### 경과 시간과 사용 시간
경과 시간
프로세스 시작부터 종료할 때까지 경과한 시간. 초시계로 프로세스 시작부터 종료할 때까지 측정한 값
경과 시간 = 사용 시간

사용 시간
프로세스가 실제로 논리 CPU를 사용한 시간

time 명령어로 얻은 user와 sys 값은 정보 확인 대상의 자식 프로세스가 실행한 시간도 포함됨

---
### 타임 슬라이스
스케줄러가 실행 가능한 프로세스에 타임 슬라이스 단위로 CPU를 나눠줌

업그레이드 버전이 목표 레이턴시

턴어라운드 타임
	처리(반환) 시간
	시스템에 처리를 요청했을 때부터 처리가 끝날 때까지 걸린 시간
